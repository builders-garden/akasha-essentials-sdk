...
export const useBlocksPublishing = (props: UseBlocksPublishingProps) => {
  ...
  const [availableBlocks, setAvailableBlocks] = React.useState<Array<any>>([]);
  ...
  const defaultTextBlock = availableBlocks.find(
    (block) => block.propertyType === DEFAULT_TEXT_BLOCK
  );
  ...
  const createContentBlocks = React.useCallback(
    async (
      nsfw: boolean,
      editorTags: string[],
      blocksWithActiveNsfw: Map<number, boolean>
    ) => {
      ...
      for (const [idx, block] of blocksInUse.entries()) {
        if (!block.status) {
          ...
          try {
            ...
            if (data && data.response && data.response.blockID) {
              setBlocksInUse((prev) => [
                ...prev.slice(0, idx),
            ...
            setErrors((prev) => [
              ...prev,
              new Error(
                `Failed to create content blocks: ${
  ...
  const addBlockToList = (
    ...
    const block = availableBlocks.find(
      (bl) =>
        bl.propertyType === selectedBlock.propertyType &&
        bl.appName === selectedBlock.appName
    );
